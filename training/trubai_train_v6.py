"""
trubai_train_v6.py
TensorConditioner v6 — Phase 3 Stabilization
SPEC-TC-V6-v1 (Muse-approved)

Delta against SPEC-TC-V5-v2:
  - Phase 1 skipped — starts from v5 epoch_050.pt (epoch 51)
  - Fresh AdamW; scheduler initialized with last_epoch=50
  - W_anchor loaded read-only from v5 phase1_anchor.pt (not recomputed)
  - Phase 3: four sub-phases 3a–3d (500→1200→2000→3181)
  - LAMBDA_ANCHOR_PHASE3 = 0.05 (named constant, flat, non-decaying)
  - Bridge trigger: pos_sim < 0.03 OR proj_norm < 32.0, 2 consecutive epochs
  - Revert table: one sub-phase back, 5 epochs, Faber applies without Muse
  - Reporting gates: epochs 130, 145, 160, 175, 200

Usage
-----
    modal run trubai_train_v6.py
    modal run trubai_train_v6.py --resume-epoch 130
"""
...
app = modal.App("trubai-train-v6", image=image)
...
class TrainV6:
    ...
    def train(self, resume_epoch: int = 0) -> None:
        ...
        start_epoch = 51
        if resume_epoch > 0:
            ckpt = V6_DIR / f"epoch_{resume_epoch:03d}.pt"
            if not ckpt.exists():
        ...
        # ── Bridge state ──────────────────────────────────────────────────────
        trigger_consecutive  = 0    # resets when trigger condition clears
        revert_active        = False
        revert_k             = 0
        ...
        for epoch in range(start_epoch, 201):
            ...
            # ── Logging (Phase 2: every 10; Phase 3: every epoch) ─────────────
            if phase == 2 and epoch % 10 == 0:
            ...
            if epoch in (130, 145, 160, 175, 200):
                torch.save(tc.state_dict(), V6_DIR / f"epoch_{epoch:03d}.pt")
                vol.commit()
            ...
            # ── Phase 2 end — epoch 130 (mandatory stop before logging here) ──
            if epoch == 130:
                anc_dist_130 = compute_anchor_loss(tc, W_anchor, 1.0).item()
                self._report_gate(
                    label="EPOCH 130 — Phase 2 end",
                    epoch=epoch, pos=last_pos, neg=last_neg,
                    gap=gap, norm=last_norm, anc_dist=anc_dist_130,
                    extra="Mandatory stop — log this report before continuing.",
                )
            ...
            if phase == 3:
                ...
                if trigger_consecutive >= 2:
                    ...
                    if revert_count_in_subphase[sub_phase] >= 3:
                        torch.save(
                            tc.state_dict(),
                            V6_DIR / f"epoch_{epoch:03d}_triple_fire_stop.pt"
                        )
                        vol.commit()
        ...
        # Save best
        (V6_DIR / "best").mkdir(exist_ok=True)
        torch.save(tc.state_dict(), V6_DIR / "best" / "tensor_conditioner.pt")
        vol.commit()
        print(f"\nAll checkpoints committed to {V6_DIR}")
...
def main(resume_epoch: int = 0) -> None:
    ...
    """
    TrainV6().train.remote(resume_epoch=resume_epoch)