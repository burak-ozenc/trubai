"""
trubai_train_v7.py
TensorConditioner v7 — Phase 2 Collapse Detection + Deterministic Sampling
SPEC-TC-V7-v1 (Muse-approved)

Delta against SPEC-TC-V6-v1:
  - random.seed(42) at script init — collapses are now deterministic/diagnosable
  - Phase 2 bridge trigger added (same two-condition OR as Phase 3)
  - Phase 2 bridge revert: k_diverse → 17, hold 5 epochs; triple-fire stop
  - Per-epoch logging in Phase 2 (was every 10)
  - Enhanced checkpoint policy: trigger events, all gates, best_phase2.pt
  - Reporting gates Phase 2: 60, 70, 80, 90, 100, 110, 120, 130
  - Reporting gates Phase 3: 145, 160, 175, 200

Unchanged from v6:
  - Start: v5/epoch_050.pt; W_anchor: v5/phase1_anchor.pt (read-only)
  - LAMBDA_ANCHOR_PHASE3 = 0.05 flat
  - Phase 3 sub-phase ramp 3a–3d (500→1200→2000→3181)
  - W_negative 19 tokens, TensorConditioner architecture

Usage
-----
    modal run trubai_train_v7.py
    modal run trubai_train_v7.py --resume-epoch 90
"""
...
app = modal.App("trubai-train-v7", image=image)
...
# v5 checkpoints — read-only in v6
V5_EPOCH50  = CKPT_DIR / "retrain_v5" / "epoch_050.pt"
V5_ANCHOR   = CKPT_DIR / "retrain_v5" / "phase1_anchor.pt"

# v6 output dir
V7_DIR = CKPT_DIR / "retrain_v7"
...
class TrainV7:
    ...
    def setup(self):
        ...
        # ── Prerequisite check — v5 checkpoints must exist ───────────────────
        for path in (V5_EPOCH50, V5_ANCHOR):
            if not path.exists():
                raise FileNotFoundError(
                    f"v5 prerequisite checkpoint missing: {path}\n"
                    "v6 cannot begin. Confirm both v5 checkpoints exist on the volume."
                )
        print("v5 prerequisite checkpoints confirmed ✓")
        print(f"  {V5_EPOCH50}")
        print(f"  {V5_ANCHOR}")
    ...
    def train(self, resume_epoch: int = 0) -> None:
        ...
        # ── TensorConditioner — load v5 epoch 50 weights ────────────────────
        tc = build_tensor_conditioner(str(self.device))
        tc.load_state_dict(
            torch.load(V5_EPOCH50, map_location=self.device, weights_only=True)
        )
        print(f"  Loaded v5 epoch_050.pt ✓")
        ...
        # ── Optimizer — fresh AdamW, no v5 Phase 1 state ────────────────────
        # v5 Phase 1 ran without L_anchor. Stale momentum would corrupt Phase 2.
        LR_MAX      = 4.5e-5
        LAMBDA_NORM = 0.1
        TARGET_NORM = 36.5
        ...
        # ── Scheduler — initialized fresh, fast-forwarded 50 steps ────────
        # PyTorch requires initial_lr in param_groups when last_epoch != -1,
        # which is not set on a freshly constructed AdamW.
        # Workaround: init normally (last_epoch=-1), call step() 50 times to
        # advance internal counter to epoch 50. First training step (epoch 51)
        # will then use LR(51) ≈ 0.837 × LR_MAX — continuous with v5 Phase 1.
        ...
        expected_lr_min = LR_MAX * 0.80
        expected_lr_max = LR_MAX * 0.90
        assert expected_lr_min <= first_lr <= expected_lr_max, (
            f"Scheduler LR sanity check failed: first_lr={first_lr:.6f} "
            f"expected in [{expected_lr_min:.6f}, {expected_lr_max:.6f}]\n"
            "Fast-forward loop may have wrong step count."
        )
        print(f"  Scheduler LR at first step: {first_lr:.6f} "
        ...
        start_epoch = 51
        if resume_epoch > 0:
            ckpt = V7_DIR / f"epoch_{resume_epoch:03d}.pt"
            if not ckpt.exists():
            ...
            # Fast-forward scheduler from epoch 50 to resume_epoch
            for _ in range(resume_epoch - 50):
                scheduler.step()
            print(f"  Scheduler fast-forwarded to epoch {resume_epoch}: "
                  f"LR={scheduler.get_last_lr()[0]:.6f}")

        # ── Bridge state — Phase 2 (independent from Phase 3) ────────────────
        p2_trigger_consecutive = 0
        p2_revert_active       = False
        p2_revert_epochs_left  = 0
        p2_revert_count        = 0    # triple-fire stop at 3
        ...
        for epoch in range(start_epoch, 201):
            ...
            # ── Batch assembly ────────────────────────────────────────────────
            if phase == 2:
                if p2_revert_active and p2_revert_epochs_left > 0:
                    k = 17   # Phase 2 sub-phase 8 max (22 anchors + 17 diverse = 39)
                    p2_revert_epochs_left -= 1
                    if p2_revert_epochs_left == 0:
                        p2_revert_active = False
                        print(f"  [p2 bridge] Revert complete at epoch {epoch}. "
                              f"Resuming normal Phase 2 k.")
                else:
            ...
            # ── Logging (Phase 2: every epoch; Phase 3: every epoch) ────────────
            if phase == 2:
                anc_dist = compute_anchor_loss(tc, W_anchor, 1.0).item()
                print(
                    f"Epoch {epoch:3d} | phase=2 | k_div={k:4d} | "
                    f"gap={gap:.4f} | pos={last_pos:.4f} | neg={last_neg:.4f} | "
                    f"norm={last_norm:.2f} | λ_anc={lambda_anchor:.4f} | "
                    f"anc_dist={anc_dist:.4f} | lr={scheduler.get_last_lr()[0]:.6f}"
                )
            elif phase == 3:
                anc_dist = compute_anchor_loss(tc, W_anchor, 1.0).item()
                revert_tag = f" [REVERT {revert_epochs_left}ep]" if revert_active else ""
                print(
                    f"Epoch {epoch:3d} | {sub_phase}{revert_tag} | k_div={k:4d} | "
                    f"gap={gap:.4f} | pos={last_pos:.4f} | neg={last_neg:.4f} | "
                    f"norm={last_norm:.2f} | λ_anc={lambda_anchor:.4f} | "
                    f"anc_dist={anc_dist:.4f} | lr={scheduler.get_last_lr()[0]:.6f}"
                )

            # ── Checkpoint save policy (SPEC-TC-V7-v1) ──────────────────────
            # Gate checkpoints: all Phase 2 gates + Phase 3 boundaries
            if epoch in (60, 70, 80, 90, 100, 110, 120, 130, 145, 160, 175, 200):
                torch.save(tc.state_dict(), V7_DIR / f"gate_ep{epoch:03d}.pt")
                vol.commit()

            # best_phase2.pt — continuous, overwrite when pos_sim improves
            if phase == 2 and last_pos > best_phase2_pos:
                best_phase2_pos = last_pos
                torch.save(tc.state_dict(), V7_DIR / "best_phase2.pt")
                # no vol.commit() here — commit only at gates to avoid overhead

            # ══════════════════════════════════════════════════════════════════
            # REPORTING GATES
            # ══════════════════════════════════════════════════════════════════

            # ── Phase 2 reporting gates: 60, 70, 80, 90, 100, 110, 120, 130 ────
            if phase == 2 and epoch in (60, 70, 80, 90, 100, 110, 120, 130):
                anc_dist_gate = compute_anchor_loss(tc, W_anchor, 1.0).item()
                extra_note = ""
                if epoch == 90:
                    extra_note = "New gate — snapshot for Phase 2 health check."
                if epoch == 130:
                    extra_note = "Mandatory stop — log this report before continuing."
                self._report_gate(
                    label=f"EPOCH {epoch} — Phase 2 gate",
                    epoch=epoch, pos=last_pos, neg=last_neg,
                    gap=gap, norm=last_norm, anc_dist=anc_dist_gate,
                    extra=extra_note,
                )
                if epoch == 130:
                    print("  Phase 3 begins at epoch 131 (3a, k=500).")
            ...
            # ── Phase 2 bridge trigger (SPEC-TC-V7-v1 Change 1) ─────────────────
            # Same two-condition OR as Phase 3. Independent revert counter.
            # Revert: k_diverse → 17 (Phase 2 sub-phase 8 max), hold 5 epochs.
            if phase == 2:
                if check_bridge_trigger(last_pos, last_norm):
                    p2_trigger_consecutive += 1
                    print(f"  [p2 bridge watch] epoch {epoch}: trigger condition met "
                          f"(pos={last_pos:.4f}<0.03={last_pos<0.03} | "
                          f"norm={last_norm:.2f}<32.0={last_norm<32.0}) "
                          f"— {p2_trigger_consecutive}/2 consecutive")
                else:
                    if p2_trigger_consecutive > 0:
                        print(f"  [p2 bridge watch] epoch {epoch}: condition cleared "
                              f"— counter reset")
                    p2_trigger_consecutive = 0

                if p2_trigger_consecutive >= 2:
                    p2_revert_count       += 1
                    p2_trigger_consecutive = 0

                    # Triple-fire stop — Phase 2 independent
                    if p2_revert_count >= 3:
                        torch.save(
                            tc.state_dict(),
                            V7_DIR / f"trigger_ep{epoch:03d}_p2_triple_fire.pt"
                        )
                        vol.commit()
                        print(f"\n>>> P2 TRIPLE-FIRE STOP at epoch {epoch}.")
                        print(f"    p2_revert_count={p2_revert_count}")
                        print(f"    Checkpoint saved. Report to Muse.")
                        raise RuntimeError(
                            f"Phase 2 bridge trigger fired 3× at epoch {epoch}. "
                            "Structural problem. Report to Muse."
                        )

                    p2_revert_active      = True
                    p2_revert_epochs_left = 5

                    # Save trigger-event checkpoint
                    torch.save(
                        tc.state_dict(),
                        V7_DIR / f"trigger_ep{epoch:03d}.pt"
                    )
                    vol.commit()

                    print(f"\n>>> P2 Bridge trigger fired at epoch {epoch}.")
                    print(f"    Reverting k_diverse → 17 for 5 epochs.")
                    print(f"    pos_sim={last_pos:.4f}, proj_norm={last_norm:.2f}")
                    print(f"    p2_revert_count={p2_revert_count}")

            # ── Bridge trigger check (Phase 3 only) ───────────────────────────
            if phase == 3:
                sub_phase = get_sub_phase_3(epoch)
                if check_bridge_trigger(last_pos, last_norm):
                ...
                if trigger_consecutive >= 2:
                    ...
                    if revert_count_in_subphase[sub_phase] >= 3:
                        torch.save(
                            tc.state_dict(),
                            V7_DIR / f"epoch_{epoch:03d}_triple_fire_stop.pt"
                        )
                        vol.commit()
                    ...
                    torch.save(tc.state_dict(), V7_DIR / f"trigger_ep{epoch:03d}.pt")
                    vol.commit()
        ...
        # Save best
        (V7_DIR / "best").mkdir(exist_ok=True)
        torch.save(tc.state_dict(), V7_DIR / "best" / "tensor_conditioner.pt")
        vol.commit()
        print(f"\nAll checkpoints committed to {V7_DIR}")
...
def main(resume_epoch: int = 0) -> None:
    ...
    """
    TrainV7().train.remote(resume_epoch=resume_epoch)