"""
trubai_train_v12.py
TensorConditioner v12 — Output Normalization + Scale Decoupling
SPEC-TC-V12-v1 (Muse-approved)
...
app = modal.App("trubai-train-v12", image=image)
...
def compute_anchor_loss(model, W_anchor: dict, lambda_val: float):
    ...
    Only parameters present in W_anchor are regularized.
    output_scale is a v12-new parameter with no anchor entry — skipped.
    """
    import torch
    loss = torch.tensor(0.0, device=next(model.parameters()).device)
    for name, param in model.named_parameters():
        if name not in W_anchor:
            continue   # output_scale and any future new params — no anchor to compare
        anchor = W_anchor[name].to(param.device)
        loss   = loss + (param - anchor).pow(2).sum()
    return lambda_val * loss

def check_bridge_trigger(pos_sim: float, output_scale: float) -> bool:
    """
    Two-condition OR trigger (SPEC-TC-V12-v1).
    Condition A: pos_sim < 0.03
    Condition B: output_scale < 32.0 (replaces proj_norm — same threshold, new target)
    """
    return (pos_sim < 0.03) or (output_scale < 32.0)
...
def build_conditioner_v12(device: str):
    ...
    class ConditionerV12(nn.Module):
        def __init__(self):
            ...
            # CONFIRMATION 1: nn.Parameter → in optimizer param group, updated by AdamW
            self.output_scale = nn.Parameter(torch.tensor(36.5))
...
class TrainV12:
    ...
    def train(self, resume_epoch: int = 0) -> None:
        ...
        start_epoch = 131
        if resume_epoch > 0:
            ckpt = V12_DIR / f"epoch_{resume_epoch:03d}.pt"
            if not ckpt.exists():
        ...
        for epoch in range(start_epoch, 201):  # Phase 3 only: 131–200
            ...
            for record in epoch_data:
                ...
                L_c  = -(pos_sim - neg_sim)
                # L_norm drives output_scale (magnitude) only — never touches direction
                L_n  = LAMBDA_NORM * (output_scale - TARGET_NORM) ** 2
                L_a  = compute_anchor_loss(tc, W_anchor, lambda_anchor)
                # L_guard removed — crash mechanism eliminated structurally (SPEC-TC-V12-v1)
                loss = L_c + L_n + L_a
                loss.backward()
            ...
            if phase == 3:
                ...
                if trigger_consecutive >= 2:
                    ...
                    if revert_count_in_subphase[sub_phase] >= 3:
                        torch.save(
                            tc.state_dict(),
                            V12_DIR / f"epoch_{epoch:03d}_triple_fire_stop.pt"
                        )
                        vol.commit()
                    ...
                    torch.save(tc.state_dict(), V12_DIR / f"trigger_ep{epoch:03d}.pt")
                    vol.commit()
                    ...
                    print(f"    pos_sim={last_pos:.4f}, output_scale={last_scale:.2f}")
        ...
        print(f"  output_scale = {last_scale:.2f}  (target 35.0–38.0)")
        ...
        criteria = {
            ...
            "output_scale 35–38":  35.0 <= last_scale <= 38.0,
        ...
        # Save best
        (V12_DIR / "best").mkdir(exist_ok=True)
        torch.save(tc.state_dict(), V12_DIR / "best" / "tensor_conditioner.pt")
        vol.commit()
        print(f"\nAll checkpoints committed to {V12_DIR}")
...
def main(resume_epoch: int = 0) -> None:
    ...
    """
    TrainV12().train.remote(resume_epoch=resume_epoch)