"""
trubai_train_v8.py
TensorConditioner v8 — Phase 3 Direct-Start + Gradient Clipping
SPEC-TC-V8-v1 (Muse-approved)
...
app = modal.App("trubai-train-v8", image=image)
...
class TrainV8:
    ...
    def train(self, resume_epoch: int = 0) -> None:
        ...
        start_epoch = 131
        if resume_epoch > 0:
            ckpt = V8_DIR / f"epoch_{resume_epoch:03d}.pt"
            if not ckpt.exists():
            ...
            if phase == 3:
                ...
                if trigger_consecutive >= 2:
                    ...
                    if revert_count_in_subphase[sub_phase] >= 3:
                        torch.save(
                            tc.state_dict(),
                            V8_DIR / f"epoch_{epoch:03d}_triple_fire_stop.pt"
                        )
                        vol.commit()
                    ...
                    torch.save(tc.state_dict(), V8_DIR / f"trigger_ep{epoch:03d}.pt")
                    vol.commit()
        ...
        # Save best
        (V8_DIR / "best").mkdir(exist_ok=True)
        torch.save(tc.state_dict(), V8_DIR / "best" / "tensor_conditioner.pt")
        vol.commit()
        print(f"\nAll checkpoints committed to {V8_DIR}")
...
def main(resume_epoch: int = 0) -> None:
    ...
    """
    TrainV8().train.remote(resume_epoch=resume_epoch)