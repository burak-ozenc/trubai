"""
trubai_train_v11.py
TensorConditioner v11 — L_guard + Universal k=17 Revert
SPEC-TC-V11-v1 (Muse-approved)
...
app = modal.App("trubai-train-v11", image=image)
...
LAMBDA_GUARD         = 2.0   # SPEC-TC-V11-v1: one-sided norm guard
# L_guard = LAMBDA_GUARD * clamp(35.0-norm, 0)^2
# Zero at norm >= 35, quadratic below threshold
...
def get_k_diverse_phase3(epoch: int) -> int:
    """
    SPEC-TC-V10-v1 Phase 3 ramp.
    Intra-3a staged: k=40→70→110→170→250 (3 epochs each stage).
    3b: k=500 (epoch 146–160)
    3c: k=1200 (epoch 161–175)
    3d: k=3181 (epoch 176–200)
    Anchor % at each stage: 35.5% / 23.9% / 16.7% / 11.5% / 8.1%
    """
    if epoch < 131:
        raise ValueError(f"get_k_diverse_phase3 called before Phase 3 (epoch={epoch})")
    # Intra-3a stages (3 epochs each)
    if epoch <= 133: return 40     # 3a-1: 35.5% anchor
    if epoch <= 136: return 70     # 3a-2: 23.9% anchor
    if epoch <= 139: return 110    # 3a-3: 16.7% anchor
    if epoch <= 142: return 170    # 3a-4: 11.5% anchor
    if epoch <= 145: return 250    # 3a-5:  8.1% anchor
    # Post-3a ramp
    if epoch <= 160: return 500    # 3b
    if epoch <= 175: return 1200   # 3c
    return 3181                    # 3d

def get_sub_phase_3(epoch: int) -> str:
    """Returns sub-phase label for logging."""
    if epoch <= 133: return "3a-1"
    if epoch <= 136: return "3a-2"
    if epoch <= 139: return "3a-3"
    if epoch <= 142: return "3a-4"
    if epoch <= 145: return "3a-5"
    if epoch <= 160: return "3b"
    if epoch <= 175: return "3c"
    return "3d"

# 3a gate epochs for per-gate proj_norm hold check (SPEC-TC-V10-v1)
INTRA_3A_GATES = {133, 136, 139, 142, 145}
INTRA_3A_GATE_LABELS = {
    133: "3a-1 end (k=40→70)",
    136: "3a-2 end (k=70→110)",
    139: "3a-3 end (k=110→170)",
    142: "3a-4 end (k=170→250)",
    145: "3a-5 end (k=250→500)",
}
...
# Bridge revert table — universal k=17 (SPEC-TC-V11-v1)
# k=17 is the ONLY empirically confirmed stable recovery baseline.
# k=250 as 3b revert was a spec error — k=250 is not stable.
# No sub-phase level is a valid revert target unless confirmed stable in a prior run.
...
class TrainV11:
    ...
    def train(self, resume_epoch: int = 0) -> None:
        ...
        start_epoch = 131
        if resume_epoch > 0:
            ckpt = V11_DIR / f"epoch_{resume_epoch:03d}.pt"
            if not ckpt.exists():
        ...
        revert_count_in_subphase: dict[str, int] = {
            "3a-1": 0, "3a-2": 0, "3a-3": 0, "3a-4": 0, "3a-5": 0,
            "3b": 0, "3c": 0, "3d": 0,
        }
        extended_once: dict[str, bool] = {
            "3a-1": False, "3a-2": False, "3a-3": False, "3a-4": False, "3a-5": False,
            "3b": False, "3c": False, "3d": False,
        }

        # ── best_phase3.pt tracking ────────────────────────────────────────────
        best_phase3_pos = 0.0

        # ── Intra-3a gate hold state ──────────────────────────────────────────
        # When proj_norm < 34 at a 3a gate, hold k for 3 extra epochs.
        # hold_active and hold_epochs_left track this; hold_k is k to hold at.
        hold_active      = False
        hold_epochs_left = 0
        hold_k           = 0
        ...
        for epoch in range(start_epoch, 201):  # Phase 3 only: 131–200
            ...
            elif hold_active and hold_epochs_left > 0:
                k = hold_k
                hold_epochs_left -= 1
                if hold_epochs_left == 0:
                    hold_active = False
                    print(f"  [3a hold] Hold complete at epoch {epoch}. "
                          f"Resuming normal ramp.")
            else:
            ...
            for record in epoch_data:
                ...
                # L_guard: one-sided norm guard (SPEC-TC-V11-v1)
                # Fires only when proj_norm < 35.0, zero above threshold.
                # Quadratic: gradient = LAMBDA_GUARD * 2 * (35 - norm)
                L_g  = LAMBDA_GUARD * torch.clamp(35.0 - proj_norm, min=0.0) ** 2
                loss = L_c + L_n + L_a + L_g
                loss.backward()
            ...
            import torch as _t
            l_guard_val = (LAMBDA_GUARD * _t.clamp(35.0 - _t.tensor(last_norm), min=0.0) ** 2).item()
            ...
            print(
                ...
                f"norm={last_norm:.2f} | Lg={l_guard_val:.3f} | "
                f"anc_dist={anc_dist:.4f} | "
                f"epoch_lr={epoch_lr:.2e} | step_lr={step_lr_log:.2e}"
            )

            # ── Checkpoint save policy (SPEC-TC-V10-v1) ─────────────────────
            # Gate checkpoints: all intra-3a gates + post-3a boundaries
            if epoch in INTRA_3A_GATES or epoch in (160, 175, 200):
                torch.save(tc.state_dict(), V11_DIR / f"gate_ep{epoch:03d}.pt")
                vol.commit()

            # best_phase3.pt — continuous, overwrite when pos_sim improves
            if last_pos > best_phase3_pos:
                best_phase3_pos = last_pos
                torch.save(tc.state_dict(), V11_DIR / "best_phase3.pt")

            # ══════════════════════════════════════════════════════════════════
            # REPORTING GATES
            # ══════════════════════════════════════════════════════════════════

            # ── Intra-3a gates + post-3a boundary gates ──────────────────────
            if epoch in INTRA_3A_GATES or epoch in (160, 175):
                anc_dist_now = compute_anchor_loss(tc, W_anchor, 1.0).item()
                if epoch in INTRA_3A_GATES:
                    label = f"EPOCH {epoch} — {INTRA_3A_GATE_LABELS[epoch]}"
                else:
                    label_map = {160: "3b end", 175: "3c end"}
                ...
                # Intra-3a soft hold check (SPEC-TC-V10-v1):
                # If proj_norm < 34 at any 3a gate, hold k for 3 more epochs.
                # This is not the bridge trigger — no revert, no stop. Just pause.
                if epoch in INTRA_3A_GATES and last_norm < 34.0 and not hold_active:
                    hold_active      = True
                    hold_epochs_left = 3
                    hold_k           = k   # hold at current k
                    print(f"  [3a hold] proj_norm={last_norm:.2f} < 34.0 at gate ep{epoch}.")
                    print(f"    Holding k={hold_k} for 3 epochs before next k increase.")
                    print(f"    Report to Muse — do not advance k without confirmation.")
            ...
            if phase == 3:
                ...
                if trigger_consecutive >= 2:
                    ...
                    if revert_count_in_subphase[sub_phase] >= 3:
                        torch.save(
                            tc.state_dict(),
                            V11_DIR / f"epoch_{epoch:03d}_triple_fire_stop.pt"
                        )
                        vol.commit()
                    ...
                    torch.save(tc.state_dict(), V11_DIR / f"trigger_ep{epoch:03d}.pt")
                    vol.commit()
        ...
        # Save best
        (V11_DIR / "best").mkdir(exist_ok=True)
        torch.save(tc.state_dict(), V11_DIR / "best" / "tensor_conditioner.pt")
        vol.commit()
        print(f"\nAll checkpoints committed to {V11_DIR}")
...
def main(resume_epoch: int = 0) -> None:
    ...
    """
    TrainV11().train.remote(resume_epoch=resume_epoch)