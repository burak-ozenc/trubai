"""
trubai_train_v10.py
TensorConditioner v10 — Intra-3a Staged Ramp
SPEC-TC-V10-v1 (Muse-approved)
...
app = modal.App("trubai-train-v10", image=image)
...
# 3a stages all revert to k=17 (Phase 2 max, 39 pairs total — stable baseline)
BRIDGE_REVERT: dict[str, tuple[int, str]] = {
    "3a-1": (17, "Phase 2 k=17"),
    "3a-2": (17, "Phase 2 k=17"),
    "3a-3": (17, "Phase 2 k=17"),
    "3a-4": (17, "Phase 2 k=17"),
    "3a-5": (17, "Phase 2 k=17"),
    "3b":   (250, "3a-5 k=250"),
    "3c":   (500, "3b k=500"),
    "3d":   (1200, "3c k=1200"),
}
...
class TrainV10:
    ...
    def train(self, resume_epoch: int = 0) -> None:
        ...
        start_epoch = 131
        if resume_epoch > 0:
            ckpt = V10_DIR / f"epoch_{resume_epoch:03d}.pt"
            if not ckpt.exists():
            ...
            if phase == 3:
                ...
                if trigger_consecutive >= 2:
                    ...
                    if revert_count_in_subphase[sub_phase] >= 3:
                        torch.save(
                            tc.state_dict(),
                            V10_DIR / f"epoch_{epoch:03d}_triple_fire_stop.pt"
                        )
                        vol.commit()
                    ...
                    torch.save(tc.state_dict(), V10_DIR / f"trigger_ep{epoch:03d}.pt")
                    vol.commit()
        ...
        # Save best
        (V10_DIR / "best").mkdir(exist_ok=True)
        torch.save(tc.state_dict(), V10_DIR / "best" / "tensor_conditioner.pt")
        vol.commit()
        print(f"\nAll checkpoints committed to {V10_DIR}")
...
def main(resume_epoch: int = 0) -> None:
    ...
    """
    TrainV10().train.remote(resume_epoch=resume_epoch)