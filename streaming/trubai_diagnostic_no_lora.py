# TRUB.AI — Phase 6, Scholar Diagnostic
# Base Moshi + TensorConditioner only. No LoRA adapters.
# Purpose: isolate whether attractor words come from LoRA (Hypothesis A)
# or from TensorConditioner's projection region (Hypothesis B).
# ─────────────────────────────────────────────────────────────────────────────
...
app = modal.App("trubai-diagnostic")
...
# ─────────────────────────────────────────────────────────────────────────────
# Phrase observation helpers (pure numpy, no torch)
# ─────────────────────────────────────────────────────────────────────────────
...
class TrubAIDiagnostic:
    ...
    def load_models(self):
        ...
        # 1. Base Moshi — no LoRA, no merge
        mimi_weight  = hf_hub_download(loaders.DEFAULT_REPO, loaders.MIMI_NAME)
        moshi_weight = hf_hub_download(loaders.DEFAULT_REPO, loaders.MOSHI_NAME)
        self.mimi = loaders.get_mimi(mimi_weight, device=self.device)
        self.mimi.set_num_codebooks(8)
        self.merged_model = loaders.get_moshi_lm(
            moshi_weight, device=self.device, dtype=torch.bfloat16
        )
        # NOTE: self.merged_model IS the base model here — no LoRA applied.
        # Named "merged_model" to keep the rest of the session code identical.

        # 2. Wire fuser (identical to training / Option 2a setup)
        ...
        # 3. Load TensorConditioner checkpoint (our trained projector)
        # Phase 1 retrained checkpoint — contrastive + diversity loss, epoch 41
        RETRAIN_CKPT = Path("/checkpoints/retrain_v2/best/tensor_conditioner.pt")
        state = torch.load(str(RETRAIN_CKPT), map_location=self.device, weights_only=False)
        self.tensor_conditioner.load_state_dict(state)
        self.tensor_conditioner.to(torch.bfloat16)

        # 4. NO LoRA — this is the entire point of the diagnostic.
        #    Steps 4 (get_peft_model / load_adapter / merge_and_unload) are intentionally absent.
        ...
        print("Diagnostic models loaded ✓  [NO LoRA — retrained TensorConditioner (epoch 41, gap=0.39)]")

    # ── Internal helpers (identical to Option 2a) ─────────────────────────────
    ...
    @modal.method()
    def run_diagnostic(self, audio_bytes: bytes, temp: float = 0.8, temp_text: float = 0.7) -> dict:
        ...
        print(f"Audio: {len(audio)/MOSHI_SR:.1f}s | temp={temp} | mode=DIAGNOSTIC_NO_LORA")
        ...
        lm_gen  = LMGen(
            ...
            temp=temp,
            temp_text=temp_text,
            # No on_text_logits_hook — clean generation, no intervention
        )
...
def main():
    ...
    result = TrubAIDiagnostic().run_diagnostic.remote(audio_bytes, temp=0.8, temp_text=0.7)

    print("\n" + "="*60)
    print("DIAGNOSTIC RESULTS — NO LORA")
    print("="*60)
    ...
    print("  PASS: attractor words (proud/brittle/ville/nik) disappear or weaken,")
    print("        AND/OR trumpet vocabulary (air/tone/column/breath) appears in any phrase window.")
    print("        Even one trumpet token = direction change is reaching generation → proceed to Phase 2 LoRA.")
    print("  FAIL: same attractor words, no trumpet vocabulary → gap=0.39 insufficient, report to Scholar.")