# TRUB.AI — Streaming Session v3
# SPEC-5A-v1: PhraseConditioner integrated, forced_text_token queue drain.
# Gate configurable via SILENCE_GATE_SECS (default 1.5s, Muse spec).
# ─────────────────────────────────────────────────────────────────────────────
#
# REQUIRES: moshi installed from TRUB.AI fork (not upstream PyPI)
# The fork adds forced_text_token parameter to LMGen._step() and step().
# Install via image:
#   .pip_install(["git+https://github.com/YOUR_FORK/moshi.git#subdirectory=moshi"])
# ─────────────────────────────────────────────────────────────────────────────
...
app = modal.App("trubai-streaming-v3")

# ── Update this URL once fork is pushed ──────────────────────────────────────
...
FRAME             = 1920
RMS_THRESH        = 0.01
SILENCE_GATE_SECS = 1.5   # Muse spec: configurable, default 1.5s
# Live performance: 1.5s
# Test recordings with deliberate pauses: 0.6s
...
# ─────────────────────────────────────────────────────────────────────────────
# Phrase observation helpers (pure numpy)
# ─────────────────────────────────────────────────────────────────────────────
...
class TrubAI:
    ...
    def load_models(self):
        ...
        # 6. Text tokenizer — path also passed to PhraseConditioner
        ...
        self.tokenizer_path = text_tok_path  # stored for PhraseConditioner
    ...
    def run_session(
        ...
        # Import PhraseConditioner — copied to /root at image build time
        ...
        # PhraseConditioner — one instance per session (SPEC-5A Section 5.1)
        ...
        with torch.no_grad(), lm_gen.streaming(1), self.mimi.streaming(1):
            ...
            def process_frame(frame):
                ...
                if phrase_fired is not None and phrase_idx < 20:
                    ...
                    # Build feature vectors for phrase (same as observe_phrase)
                    ...
                    logs.append({
                        ...
                        "phrase_features":   {"pitch_accuracy": round(pf.pitch_accuracy, 3),
                                              "tone_quality":   round(pf.tone_quality, 3)} if pf else None,
                    })
                    current_phrase  = phrase_idx
                    phrase_idx     += 1

                # SPEC-5A Section 5.3 — queue drain on every step()
                chunk = torch.from_numpy(frame).float().unsqueeze(0).to(self.device)
                codes = self.mimi.encode(chunk.unsqueeze(0))
                if codes.shape[-1] > 0:
                    for t in range(codes.shape[-1]):
                        # Get next forced token (None = free sampling)
                        forced = phrase_conditioner.next_token(self.device)
                        result = lm_gen.step(
                            codes[:, :, t:t+1],
                        ...
                        if result is not None:
                            ...
                            if tok_id > 3:
                                ...
                                text_buffer.append((piece, is_forced))
        ...
        return {
            "logs":          logs,
            "text_tokens":   text_buffer,    # list of (piece, is_forced)
...
def main():
    ...
    # Use 0.6s gate so all four phrase windows fire with post-boundary audio
    result = TrubAI().run_session.remote(
        audio_bytes, temp=0.8, temp_text=0.7, silence_gate_secs=0.6
    )

    print("\n" + "="*60)
    print("SESSION RESULTS — SPEC-5A-v1")
    print("="*60)
    ...
    print(" ".join(
        f"[F]{piece}" if is_forced else piece
        for piece, is_forced in result["text_tokens"]
    ))
