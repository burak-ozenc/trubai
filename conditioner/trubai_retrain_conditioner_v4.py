...
# W_negative existing — 11 unique IDs (12 words, one collision after dedup)
...
def build_tensor_conditioner(prod_state_dict: dict):
    """
    Construct a TensorConditioner whose architecture matches the production
    checkpoint (retrain_v2/best). Infers in_dim and hid_dim from weight shapes,
    asserts key/shape parity. Returns a freshly initialized (random) instance.
    """
    import torch
    import torch.nn as nn
    import torch.nn.functional as F

    proj_w = prod_state_dict.get("projection.weight")
    if proj_w is None:
        keys = list(prod_state_dict.keys())
        raise KeyError(
            f"'projection.weight' not found in production checkpoint.\n"
            f"Available keys: {keys}\n"
            f"Architecture may differ — report to Muse."
        )

    in_dim  = proj_w.shape[1]   # expected 768
    hid_dim = proj_w.shape[0]   # expected 4096
    has_out = "output_proj.weight" in prod_state_dict

    print(f"  TensorConditioner: in={in_dim} → hid={hid_dim}, "
          f"output_proj={'yes' if has_out else 'no'}")
    ...
    class TensorConditioner(nn.Module):
        ...
        def forward(
                self,
                emb: "torch.Tensor",         # [1, 1, 768]
                W_pos: "torch.Tensor",        # [n_pos, hid_dim]
                W_neg: "torch.Tensor",        # [n_neg, hid_dim]
        ):
            x = emb.squeeze(1)            # [1, 768]
            x = self.projection(x)        # [1, hid_dim]
            if has_out:
                x = self.output_proj(x)   # [1, hid_dim]
    ...
    test_state = tc.state_dict()

    for k in test_state:
        assert k in prod_state_dict, (
            f"Architecture mismatch: built key {k!r} absent from prod checkpoint.\n"
            f"Prod keys: {list(prod_state_dict.keys())}"
        )
        assert test_state[k].shape == prod_state_dict[k].shape, (
            f"Shape mismatch at {k!r}: "
            f"built {test_state[k].shape} vs prod {prod_state_dict[k].shape}"
        )

    print(f"  Architecture verified against retrain_v2/best ✓")
    return tc   # random initialization — NOT loaded from prod checkpoint
...
class TrainV5:
    ...
    def setup(self):
        ...
        from moshi.loaders import MOSHI_MODEL_ID
        ...
        # ── Moshi text embedding table ───────────────────────────────────────
        # Cache the embedding matrix on the volume so subsequent startups
        # skip the 7B model load entirely.
        if EMB_CACHE_PATH.exists():
            print("Loading cached embedding table...")
            emb_weight = torch.load(EMB_CACHE_PATH, map_location="cpu",
                                    weights_only=True)
        else:
            print("Extracting embedding table from Moshi LM (first time — slow)...")
            moshi_lm   = get_moshi_lm(repo_id=MOSHI_MODEL_ID, device="cpu")
            emb_weight = moshi_lm.lm_model.emb.weight.detach().float()
            del moshi_lm
            torch.cuda.empty_cache()
            torch.save(emb_weight, EMB_CACHE_PATH)
            vol.commit()
            print(f"  Embedding table cached at {EMB_CACHE_PATH}")

        print(f"  Embedding table: {emb_weight.shape}")   # expected [vocab, 4096]

        # ── W_positive — 18 verified IDs ────────────────────────────────────
        pos_ids      = list(W_POSITIVE_IDS.values())
        self.W_pos   = emb_weight[pos_ids].to(self.device)   # [18, 4096]

        # ── W_negative — built after verify_tokens(), stored here ────────────
        # Populated in verify_and_build_wneg(); kept None until then.
        self.W_neg         = None
        self.emb_weight    = emb_weight   # full table — sliced during verify
    ...
    def train(
        ...
        self.W_neg = self.emb_weight[all_neg_ids].to(self.device)
        ...
        prod_state = torch.load(PROD_CKPT, map_location="cpu", weights_only=True)
        tc         = build_tensor_conditioner(prod_state).to(self.device)
        ...
        for epoch in range(start_epoch, 201):
            ...
            for record in epoch_data:
                ...
                pos_sim, neg_sim, proj_norm = tc(emb, self.W_pos, self.W_neg)
            ...
            # Trigger: gap < 0.32 (basis: v3 peak 0.3728, floor 0.37-0.05)
            if epoch == THE_GATE_EPOCH and not drop_the:
                print(f"\n--- ▁the monitoring gate (epoch {THE_GATE_EPOCH}) ---")
                print(f"    gap={gap:.4f}  threshold={THE_GATE_THRESHOLD}")
                if gap < THE_GATE_THRESHOLD:
                    # Save pre-reset state for Muse review
                    torch.save(
                        tc.state_dict(),
                        V5_DIR / "epoch_005_pre_reset.pt"
                    )
                    vol.commit()
                    print(f"\n>>> ▲ ▁the GATE TRIGGERED")
                    print(f"    gap={gap:.4f} < {THE_GATE_THRESHOLD}")
                    print(f"    epoch_005_pre_reset.pt saved.")
                    print(f"    Report to Muse. Do not reset without confirmation.")
                    print(f"    When Muse approves: rerun with --drop-the 1")
                    raise RuntimeError(
                        f"▁the gate triggered at epoch 5: "
                        f"gap={gap:.4f} < {THE_GATE_THRESHOLD}. "
                        "Stopped. Report to Muse before resetting."
                    )
                else:
                    print(f"    gap ≥ {THE_GATE_THRESHOLD} — ▁the confirmed safe ✓")
                    print(f"    No further ▁the monitoring needed.")
...
def main(
    ...
    if mode == "verify":
        ...
        print("Valid modes: 'verify', 'train'")